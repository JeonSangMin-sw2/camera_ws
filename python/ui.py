...
def get_arm_config(model, arm: str):
    ...
    arm: "right" or "left"
    returns:
      arm_idx (array-like indices length >=7),
      ee_link (str)
    """
    right_idx = model.right_arm_idx

    # left_arm_idx가 있으면 우선 사용
    left_idx = getattr(model, "left_arm_idx", None)

    # 없으면 right 0~6 다음이 left 0~6이라는 전제로 생성
    if left_idx is None:
        left_idx = np.array(right_idx[:7]) + 7

    if arm == "right":
        return np.array(right_idx), "ee_right"
    elif arm == "left":
        return np.array(left_idx), "ee_left"
    else:
        raise ValueError(f"Unknown arm: {arm}")


# ============================================================
# Marker transform (네 코드에 이미 존재한다고 가정)
#   - live 모드에서만 필요
...
def run_pipeline(ip: str,
    ...
    if mode == "live":
        ...
        marker_transform = marker_transform_factory()
...
class CalibUI(tk.Tk):
    def __init__(self):
        ...
        self.var_ip = tk.StringVar(value="192.168.30.1:50051")
