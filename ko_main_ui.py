...
class MarkerRecognitionProblemDialog(QDialog):
    def __init__(self, parent=None, is_ko=True):
        super().__init__(parent)
        self.is_ko = is_ko
...
class UnifiedCalibrationApp(QWidget):
    ...
    def update_robot_connection_status(self):
        ...
        if is_connected:
            ...
            if hasattr(self, 'wizard_widget') and self.wizard_widget:
                ...
                self.wizard_widget.lbl_step2_status.setText("상태: 성공 - 로봇 연결 완료")
                self.wizard_widget.lbl_step2_status.setStyleSheet("color: #4caf50; font-weight: bold; font-size: 16px;")
                self.wizard_widget.mark_step_completed(4, True, "연결됨")
        else:
            self.btn_connect.setText("CONNECT")
            self.btn_connect.setStyleSheet("background-color: #ff9800; color: #000000; font-weight: bold; padding: 4px 8px; font-size: 11px;")
            self.btn_connect.setEnabled(True)
            if hasattr(self, 'wizard_widget') and self.wizard_widget:
                self.wizard_widget.btn_wizard_connect.setText("CONNECT (로봇 연결)")
                ...
                self.wizard_widget.lbl_step2_status.setText("상태: 연결 해제됨")
                self.wizard_widget.lbl_step2_status.setStyleSheet("color: #aaaaaa; font-size: 16px; font-weight: bold;")
                self.wizard_widget.mark_step_completed(4, False, "연결 해제됨")
            ...
            try:
                ...
                if hasattr(self, 'marker_calibrator') and self.marker_calibrator:
                    try:
                        ...
                        self.log_msg(f"[WARN] 표준 목표 관절 각도 설정 실패: {e}")
    ...
    def home_offset_reset(self, confirm_dialog=True) -> bool:
        if not self.ui_only and not self.robot:
            QMessageBox.critical(self, "오류", "로봇이 연결되어 있지 않습니다.")
            return False

        if confirm_dialog:
            msg = (
                "경고: 홈 오프셋 초기화는 로봇 관절의 영점 오프셋 위치를 물리적으로 재설정합니다.\n\n"
                "단계:\n"
                "1. 직접 교시를 사용하여 양팔을 영점 위치에 맞춰주십시오.\n"
                "2. 헤드 오프셋을 리셋하려는 경우 헤드 정면을 조준하였는지 확인하십시오.\n"
                "3. OK를 클릭하여 프로세스를 시작하십시오.\n\n"
                "진행 중 제어 매니저가 비활성화되고, 전원이 다시 켜진 후 로봇 연결이 자동 재시작됩니다."
            )
            dialog = QDialog(self)
            dialog.setWindowTitle("홈 오프셋 리셋 확인")
            ...
            if not pixmap.isNull():
                ...
                img_label.setText("[img/home_offset_position.png 찾을 수 없음]")
    ...
    def apply_full_auto_results(self):
        reply = QMessageBox.question(self, '풀 오토 결과 적용', 
                                     "보정된 관절 오프셋 및 카메라 브라켓 설정을 setting.yaml에 영구 저장 및 적용하시겠습니까?",
        ...
        if reply == QMessageBox.Yes:
            ...
            QMessageBox.information(self, "적용 완료", "모든 풀 오토 캘리브레이션 결과가 성공적으로 적용되었습니다.")
