...
def main():
    ...
    def run_opt(lambda_cam_rot, lambda_cam_pos):
    ...
    # Cases
    cases = [
        ("Camera rotation FREE (standard)", 0.0, 0.0),
        ("Camera rotation LOCKED to CAD", 100000.0, 0.0),
        ("Camera rotation & translation BOTH LOCKED to CAD", 100000.0, 100000.0)
    ]
    
    for desc, lam_rot, lam_pos in cases:
        q_offsets, T_cam = run_opt(lam_rot, lam_pos)
