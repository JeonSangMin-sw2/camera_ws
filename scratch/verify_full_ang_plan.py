...
def main():
    ...
    robot_holder = rby.create_robot("127.0.0.1", "m")
    class MockRobot:
        def model(self): return robot_holder.model()
        def get_dynamics(self): return robot_holder.get_dynamics()
        def get_state(self):
    ...
    res_str = []
    res_str.append("=================================================================")
    res_str.append(" VERIFICATION OF ALL 14 JOINTS WITH FULL_ANG (±5.0°) SWEEPS")
    res_str.append("=================================================================")
    res_str.append(" [RIGHT ARM]")
    for i in range(7):
        status = "[PASS < 0.08°]" if err_r[i] < 0.08 else "[EXCEEDS 0.08°]"
        res_str.append(f"   J{i}: GT = {gt_r[i]:+6.2f}° | Calc = {calc_r[i]:+8.4f}° | Error = {err_r[i]:6.4f}° {status}")

    res_str.append("\n [LEFT ARM]")
    for i in range(7):
        status = "[PASS < 0.08°]" if err_l[i] < 0.08 else "[EXCEEDS 0.08°]"
        res_str.append(f"   J{i}: GT = {gt_l[i]:+6.2f}° | Calc = {calc_l[i]:+8.4f}° | Error = {err_l[i]:6.4f}° {status}")

    res_str.append("\n [HEAD]")
    res_str.append(f"   Pan : GT = {gt_h[0]:+6.2f}° | Calc = {calc_h[0]:+8.4f}° | Error = {err_h[0]:6.4f}°")
    res_str.append(f"   Tilt: GT = {gt_h[1]:+6.2f}° | Calc = {calc_h[1]:+8.4f}° | Error = {err_h[1]:6.4f}°")
    res_str.append("=================================================================")
    res_str.append(f"Max Right Joint Error : {np.max(err_r):.4f}°")
    res_str.append(f"Max Left Joint Error  : {np.max(err_l):.4f}°")
    res_str.append(f"Max Head Joint Error  : {np.max(err_h):.4f}°")

    output_content = "\n".join(res_str)
    with open("/tmp/full_ang_eval_res.txt", "w") as f:
