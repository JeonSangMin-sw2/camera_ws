...
def analyze_sweep_files(result_dir):
    ...
    print("="*50)
    sweep_files = glob.glob(os.path.join(result_dir, "result_txt", "sweep_points_*.txt"))
    if not sweep_files:
        print("No sweep_points_*.txt files found.")
        return
...
def analyze_convergence(result_dir):
    ...
    print("="*50)
    debug_files = glob.glob(os.path.join(result_dir, "result_txt", "joint_calib_debug_*.txt"))
    if not debug_files:
        print("No joint_calib_debug_*.txt files found.")
        return
    ...
    for fpath in sorted(debug_files):
        ...
        with open(fpath, "r") as f:
            for line in f:
                # Match: [ITERATION 1/6] Sweeping physically with staged offset -2.8683°...
                m = re.search(r"Sweeping physically with staged offset ([-0-9.]+)°", line)
                if m:
                    current_staged = float(m.group(1))
                
                # Match: * Calculated Offset Correction           : 3.209227 deg
        ...
        # Analyze divergence/vibration
        if len(iterations) > 2:
            last_corr = iterations[-1][1]
            prev_corr = iterations[-2][1]
            if abs(last_corr) > 2.0 and abs(last_corr) > abs(iterations[0][1]):
                print("  => [WARNING] Divergence detected! The correction magnitude is growing or very large.")
            elif last_corr * prev_corr < 0 and abs(last_corr) > 0.5:
