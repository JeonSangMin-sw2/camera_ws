...
def run_3stage_eval():
    ...
    # Head is NOT touched by user (Lock head offset to 0.0)
    # Joint 3 Step 1 value (1.2302 deg) tightly fixed to exact Step 1 result

    print("=========================================================================")
    print(f"EVALUATING WITH HEAD LOCKED (0.0 DEG) & EXACT STEP 1 JOINT VALUES")
    print("=========================================================================")

    # Stage 1: Global Rough Init
    opt1 = QPCalibrationOptimizer(
        ...
        lambda_cam_pos=1.0,
        lambda_cam_rot=1e6,
        optimize_head=False, # HEAD LOCKED (User did not touch head)
    ...
    opt2 = QPCalibrationOptimizer(
        ...
        lambda_cam_pos=1.0,
        lambda_cam_rot=1e6,
        optimize_head=False, # HEAD LOCKED
        optimize_camera=False, # CAMERA LOCKED
        active_arms=["right", "left"],
        estimate_measurement_noise=True,
        apply_joint_offset_limits=True,
        joint_offsets_to_apply=staged_joint_offsets,
    ...
    # Stage 3: Final Integration (eps = 1e-9)
    ...
    h_deg = np.rad2deg(q_head_final) if q_head_final is not None else [0.0, 0.0]

    print("\n" + "="*70)
    print(" RE-EVALUATION RESULTS (HEAD LOCKED TO 0.0 DEG)")
    print("="*70)
    ...
    print(f"  Head Pan            : {h_deg[0]:+.4f} deg  (Locked to 0.0)")
    print(f"  Head Tilt           : {h_deg[1]:+.4f} deg  (Locked to 0.0)")
    print("="*70)
