...
def main():
    ...
    for trial in range(N_TRIALS):
        ...
        arm_offset_true_deg = {
            "right": rng.uniform(-3, 3, 7),
            "left": rng.uniform(-3, 3, 7),
        }
        head_offset_true_deg = rng.uniform(-1.5, 1.5, 2)  # [pan, tilt] -- realistic range
        joint_noise_std_deg = rng.uniform(0, 0.1)
        cam_pos_offset_m = random_unit_vector(rng) * rng.uniform(0.01, 0.02)  # THE scenario: 1-2cm rough measurement
        cam_rot_offset_deg = rng.uniform(-1, 1, 3)
        marker_ori_noise_std_deg = rng.uniform(0, 1)
        marker_pos_noise_std_m = rng.uniform(0, 0.002)
        ...
        try:
            optimizer_pass1 = QPCalibrationOptimizer(
                ...
                camera_pos_bound_m=0.025, camera_rot_bound_rad=3.0 * D2R,
